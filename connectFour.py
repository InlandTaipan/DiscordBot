...
class connectFour:
    ...
    async def playerSelect(self, message):
        self.botMessage = await message.channel.send("Welcome to Connect 4! Those who wish to play please click on one of the coloured tokens below:")
        await self.botMessage.add_reaction(self._gameEmoji[8])
        await self.botMessage.add_reaction(self._gameEmoji[9])
...
async def run(message, client):
    game = connectFour()
    await game.playerSelect(message)
    
    await game.botMessage.edit(content = await game.getBoard())
